...
func (m *Manga4LifeParserService) GetInfo(url string) *types.MangaInfo {
...
func (m *Manga4LifeParserService) GetListChapter(url string) []types.Chapter {
	if err := m.Driver.Get(url); err != nil {
		panic(err)
	}
	...
	if chapterBoxEl, err := m.Driver.FindElement(selenium.ByCSSSelector, "body > div.container.MainContainer > div > div > div > div > div.list-group.top-10.bottom-5.ng-scope"); err == nil {
		...
		for i := len(listChapterEls) - 1; i >= 0; i-- {
			...
			chapters = append(chapters, types.Chapter{Index: int32(len(listChapterEls) - i), Name: chapterName, ChapterUrl: fmt.Sprintf("%s%s", "", chapterUrl)})
		}
	}
	return chapters
}

func (m *Manga4LifeParserService) getChapterPages(url string) ([]*types.Page, error) {
	listPages := []*types.Page{}
	if err := m.Driver.Get(url); err != nil {
	...
	if pageLayoutElement, err := m.Driver.FindElement(selenium.ByID, "TopPage"); err == nil {
		if imgElements, err := pageLayoutElement.FindElements(selenium.ByClassName, "img-fluid"); err == nil {
			for idx, el := range imgElements {
				...
				listPages = append(listPages, &types.Page{ImageUrl: src, Index: int32(idx) + 1, PageImageData: nil})
			}
		}
...
func (m *Manga4LifeParserService) ParseData(url string, start int32, end int32) *types.BookDataParseJson {
	info := m.GetInfo(url)
	...
	return &types.BookDataParseJson{
		Title:    info.Name,
